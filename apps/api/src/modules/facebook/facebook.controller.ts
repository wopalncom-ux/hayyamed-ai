...
export class FacebookController {
  ...
  constructor(private svc: FacebookService, private config: ConfigService) {}
  ...
  // Inbound Messenger events — acknowledged immediately.
  @Public()
  @Post('webhook')
  webhook(@Req() req: RawBodyRequest<Request>, @Body() body: any, @Res() res: Response) {
    ...
    if (!verifyMetaSignature(req.rawBody, signature, this.config.get('META_APP_SECRET'))) {
      this.logger.warn('❌ Rejected Facebook webhook: invalid or missing X-Hub-Signature-256')
      return res.status(403).send('Forbidden')
    }
  ...
  @Post('connect')
  connect(@CurrentUser() user: JwtPayload, @Body() body: { pageId: string; accessToken: string; pageName?: string }) {
