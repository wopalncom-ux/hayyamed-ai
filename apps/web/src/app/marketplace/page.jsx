    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#07090f', color: '#e2e8f0', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <main style={{ flex: 1, padding: '32px', overflow: 'auto' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px', marginBottom: '6px' }}>
          ...
          <a href="/clients" style={{ padding: '10px 18px', background: '#D8B16A', color: '#07090f', borderRadius: '8px', fontWeight: 800, fontSize: '13px', textDecoration: 'none', whiteSpace: 'nowrap' }}>Manage per client →</a>
        </div>
              ...
              <div key={m.key} style={{ background: '#0f1520', border: '1px solid #1a2235', borderRadius: '12px', padding: '18px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '10px' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: '11px', color: m.price > 0 ? '#fbbf24' : '#D8B16A', fontWeight: 700 }}>{m.price > 0 ? `QAR ${m.price}/mo add-on` : 'Included'}</div>
                  </div>
                </div>
                ...
                <div style={{ marginTop: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <a href="/clients" style={{ fontSize: '11px', padding: '5px 12px', background: 'rgba(216,177,106,.1)', border: '1px solid rgba(216,177,106,.3)', borderRadius: '6px', color: '#D8B16A', fontWeight: 700, textDecoration: 'none' }}>Enable for client</a>
                </div>
              </div>
            ))}
          </div>
