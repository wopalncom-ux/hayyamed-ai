...
const BASE = [
  ...
  { id: 'growth', name: 'Growth', price: 'QAR 599', period: '/month', color: '#D8B16A', popular: true,
  ...
  return (
    <section style={{ maxWidth: '960px', margin: '0 auto', padding: '80px 24px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(210px, 1fr))', gap: '14px' }}>
        {plans.map(p => (
          <div key={p.id} style={{ padding: '24px', background: p.popular ? 'rgba(216,177,106,.04)' : '#0c0f1a', border: `1px solid ${p.popular ? 'rgba(216,177,106,.25)' : '#1a2235'}`, borderRadius: '10px', position: 'relative' }}>
            {p.popular && <div style={{ position: 'absolute', top: '-12px', left: '50%', transform: 'translateX(-50%)', padding: '3px 12px', background: '#D8B16A', borderRadius: '20px', fontSize: '9px', color: '#070b0a', fontWeight: '800', whiteSpace: 'nowrap' }}>MOST POPULAR</div>}
            ...
            <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 20px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {p.features.map(f => (
                <li key={f} style={{ fontSize: '12px', color: '#94a3b8', display: 'flex', alignItems: 'flex-start', gap: '6px' }}>
                  <span style={{ color: '#D8B16A', flexShrink: 0 }}>✓</span> {f}
                </li>
              ))}
            </ul>
            <Link href="/register" style={{ display: 'block', textAlign: 'center', padding: '9px', background: p.popular ? '#D8B16A' : 'rgba(255,255,255,.04)', border: `1px solid ${p.popular ? 'transparent' : '#1a2235'}`, borderRadius: '6px', color: p.popular ? '#070b0a' : '#e2e8f0', textDecoration: 'none', fontSize: '12px', fontWeight: '700' }}>
              Get started →
            </Link>
          </div>
        ))}
